...
const badgrColor: UnionToObjectKeys<Props, "color"> = {
  ...
  black: "bg-black border-0 text-white",
...
export default function Badge({
  ...
  if (isLoading)
    return (
      <Skeleton
        width="7ch"
        className={`${loadingBadgeSize[size]} !rounded-48`}
      />
    );
  ...
  return wrapLink(
    <span className={classes} onClick={onClick}>
      ...
      {onRemove && (
        <IoMdCloseCircle onClick={onRemove} className="ml-12 cursor-pointer" />
      )}
    </span>,
    href
  );
