...
export default function ScreenshotsInput(props: Props) {
  ...
  const canUploadMore =
    uploadingCount + uploadedFiles.length < MAX_UPLOAD_COUNT;
  const placeholdersCount =
    MAX_UPLOAD_COUNT - (uploadingCount + uploadedFiles.length + 1);
        ...
        [UPLOADER_EVENTS.ITEM_FINISH]: (item) => {
          ...
          if (id && url) {
            onChange(
              [
                ...uploadedFiles,
                { id, local_id: id, name: filename, url: url },
              ].slice(-MAX_UPLOAD_COUNT)
            );
          }
        },
      }}
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-16 mt-24">
        ...
        {placeholdersCount > 0 &&
          Array(placeholdersCount)
            .fill(0)
            .map((_, idx) => <ScreenshotThumbnail key={idx} />)}
      </div>
    </Uploady>
  );
}
...
const DropZone = forwardRef<any, any>((props, ref) => {
  ...
  return (
    ...
    >
      <button className={styles.idle_content}>
        <p className="text-center text-gray-400 text-body1 mb-8">
          <FiCamera />
        </p>
        <div className={`text-gray-600 text-center text-body4`}>
          <span className="text-blue-500 underline">Browse images</span> or{" "}
          <br /> <span className="text-blue-500">drop </span>
          them here
        </div>
      </button>
      ...
      >
        Drop to upload <br />{" "}
        <AiOutlineCloudUpload className="scale-150 text-body1 mt-16" />
      </motion.div>
    </UploadDropZone>
  );
});
