          ...
          <div className="w-full h-full rounded-full relative group">
            {!img && (
              <div className="w-full h-full rounded-full bg-white hover:bg-gray-100 flex flex-col justify-center items-center">
                <p className="text-center text-gray-400 text-body2 mb-8">
                  <FiCamera />
                </p>
                <button className={`text-gray-400 text-center text-body5`}>
                  Add Image
                </button>
              </div>
            )}
                ...
                {!isUploading && (
                  <div className="flex flex-wrap gap-16 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 ">
                    <button
                      ...
                      className="py-8 px-12 rounded-full bg-black bg-opacity-70 opacity-0 group-hover:opacity-100 focus:opacity-100 hover:bg-opacity-90 transition-opacity text-white text-body1"
                    >
                      <CgArrowsExchangeV />
                    </button>
                    {props.isRemovable && (
                      <button
                        type="button"
                        className="py-8 px-12 rounded-full bg-black bg-opacity-70 opacity-0 group-hover:opacity-100 hover:bg-opacity-90 transition-opacity text-white text-body1"
                        onClick={(e) => {
                          e.stopPropagation();
                          props.onChange(null);
                        }}
            ...
            {isUploading && (
              <div className="absolute inset-0 bg-gray-400  bg-opacity-60 flex flex-col justify-center items-center text-white font-bold transition-transform">
                <RotatingLines
                  strokeColor="#fff"
                  strokeWidth="3"
