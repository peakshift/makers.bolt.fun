...
export default function TagsInput({
  ...
  const currentPlaceholder = props.placeholder ?? (
    <div className="flex gap-8 items-center text-gray-500">
      {maxReached ? "" : value.length > 0 ? "Add Another..." : placeholder}{" "}
    </div>
  );

  const tagsOptions = !maxReached
    ? (officalTags.data?.officialTags ?? [])
        .filter((t) => !value.some((v) => v.title === t.title))
        .map(transformer.tagToOption)
    : [];
...
const transformer = {
  valueToOption: (tag: Value): Option => ({
    label: formatHashtag(tag.title),
    value: tag.title,
    icon: null,
    description: null,
  }),
  tagToOption: (tag: Tag): Option => ({
    label: formatHashtag(tag.title),
    value: tag.title,
    icon: tag.icon,
    description: tag.description,
  }),
  optionToTag: (o: Option): Tag => ({
    title: o.value,
    icon: o.icon,
...
const OptionComponent = (props: OptionProps<Option>) => {
  return (
    <div>
      ...
      >
        <div
          className={`rounded-8 w-40 h-40 text-center py-8 shrink-0 bg-gray-100`}
        >
          {props.data.icon}
        </div>
        <div>
