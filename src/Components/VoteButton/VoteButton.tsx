...
export default function VoteButton({
  ...
  >([]);
  const [btnPosition, setBtnPosition] =
  ...
  const clickIncrement = () => {
    if (!disableShake)
      setBtnShakeClass((s) =>
        s === styles.clicked_2 ? styles.clicked_1 : styles.clicked_2
      );
    ...
    if (
      totalIncrementsCountRef.current &&
      totalIncrementsCountRef.current % 5 === 0
    )
      spawnSparks(5);
  ...
  return (
    ...
    >
      <div
        ...
        className={`
                ${styles.btn_content} 
                relative rounded-lg text-gray-600 ${
                  !incrementsCount && "bg-gray-50 hover:bg-gray-100"
                } 
                 ${
                   direction === "vertical"
                     ? dense
      ...
      >
        ...
        <div
          className={`
                ...
                flex justify-center items-center gap-8 text-center ${
        ...
        >
          <MdLocalFireDepartment
            className={`text-body2 ${
              incrementsCount ? "text-red-600" : "text-gray-400"
            }`}
          />
          {!hideVotesCoun && (
      ...
      <Portal id="effects-container">
        ...
        >
          {increments.map((increment) => (
            <span key={increment.id} className={styles.vote_counter}>
              +{increment.value}
            </span>
          ))}
          {sparks.map((spark) => (
            ...
            >
              <MdLocalFireDepartment className="" />
            </div>
          ))}
        </div>
      </Portal>
