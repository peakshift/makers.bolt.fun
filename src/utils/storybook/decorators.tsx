        ...
        <Provider store={store}>
          <Suspense fallback={<h2>Loading</h2>}>
            <MemoryRouter initialEntries={[modifyArgs.router?.currentPath!]}>
              <Routes>
                <Route
                  path={modifyArgs.router?.routePath}
                  element={<Story {...options} />}
                />
              </Routes>
            </MemoryRouter>
          </Suspense>
...
export const wrapModal: DecoratorFn = (Component) => (
  <Modal isOpen id="some-id" onClose={() => {}}>
    <Component />
  </Modal>
);
...
export function WrapForm<T extends FieldValues = any>(
  options?: Partial<UseFormProps<T> & { logValues: boolean }>
): DecoratorFn {
...
export function WrapFormController<T extends FieldValues = any>(
  options: Partial<UseFormProps<T> & { logValues: boolean }> & { name: string }
): DecoratorFn {
  const Func: DecoratorFn = (Story) => {
    const methods = useForm<T>(options);
