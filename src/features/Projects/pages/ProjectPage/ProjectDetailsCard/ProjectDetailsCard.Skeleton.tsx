...
export default function ProjectDetailsCardSkeleton({
  onClose,
  direction,
  ...props
}: Props) {
  ...
  return (
    ...
    >
      <div className="relative h-[100px] lg:h-[80px]">
        ...
        <button
          className="w-32 h-32  bg-gray-600 bg-opacity-80 text-white absolute top-24 right-24 rounded-full hover:bg-gray-800 text-center"
          onClick={onClose}
        >
      ...
      <div className="p-24 flex flex-col gap-24">
        <div className="flex flex-col mt-[-80px] md:flex-row md:mt-0 gap-24 items-start relative">
          <div className="flex-shrink-0 w-[108px] h-[108px] ">
            <Skeleton
              height="100%"
              className="rounded-24 border-2 border-white"
            />
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 justify-items-center">
            {Array(4)
              ...
              .map((_, idx) => (
                <div
                  key={idx}
                  className="w-full relative pt-[56%] cursor-pointer bg-gray-200 shadow-sm rounded-10 overflow-hidden"
                >
