...
export default function HeaderImage(props: Props) {
  ...
  return (
    <div className="h-[280px] rounded-20 overflow-hidden relative flex flex-col justify-center items-center gap-8">
      <img
        src={img.startsWith("https://via.placeholder.com/") ? DEFAULT_IMG : img}
        alt={`${title} cover`}
      ...
      >
        <FiArrowLeft
          className=" inline-block text-body2 lg:text-body1"
          aria-label="go back"
        />
      </Link>
      <h1 className="text-white text-[24px] md:text-[40px]  ">{title}</h1>
      {typeof apps_count === "number" && (
        <p className="text-white text-body4">{apps_count} apps</p>
      )}
    </div>
  );
}
