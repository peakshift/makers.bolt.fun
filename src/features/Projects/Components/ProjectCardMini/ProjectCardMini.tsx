...
export default function ProjectCardMini({ project, onClick }: Props) {
  return (
    <button
      className="py-16 select-none px-16 flex text-left w-full items-center gap-16 border-2 border-gray-100 rounded-16  bg-white hover:bg-gray-50"
      onClick={() => {
        onClick(project.id);
      }}
    >
      <img
        ...
        alt={project.title}
        draggable="false"
        className="flex-shrink-0 w-64 h-64 bg-gray-200 border-0 rounded-full hover:cursor-pointer"
      ></img>
