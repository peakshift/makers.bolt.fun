...
export default function MakersList(props: Props) {
  ...
  const [queryFilter, setQueryFilter] =
    useState<GetMakersInTournamentQueryVariables>({
      tournamentIdOrSlug: props.tournamentId.toString(),
  ...
  const itemsCount =
    query.data?.getMakersInTournament &&
    query.data.getMakersInTournament.makers.length;
      ...
      <div className="flex flex-col gap-16 lg:gap-24">
        {query.loading ? (
          ...
          query.data?.getMakersInTournament.makers.map((maker) => (
            <MakerCard key={maker.user.id} maker={maker} />
          ))
        ) : (
        ...
        <div className="flex justify-center gap-36 text-gray-400">
          <IconButton
            isDisabled={!query.data?.getMakersInTournament.hasPrev}
            onClick={prevPage}
          >
            <FaChevronLeft />
          </IconButton>
          <IconButton
            isDisabled={!query.data?.getMakersInTournament.hasNext}
            onClick={nextPage}
          >
            <FaChevronRight />
          </IconButton>
