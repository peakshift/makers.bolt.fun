...
export default function RegisterCard({ makers_count, start_date, avatars, isRegistered }: Props) {
    ...
    return (
        <Card onlyMd className='flex flex-col gap-24 md:!border'>
            <div>
                {makers_count > 2 && <div className="text-body5 text-gray-600 flex mb-16">
                    {avatars.map((img, idx) => <div key={idx} className='w-[16px] h-32 relative'><Avatar src={img} width={32} className='absolute top-0 left-0 min-w-[32px] !border-white' /></div>)}
            ...
            <div>
                ...
                <div className="grid grid-cols-[repeat(auto-fill,minmax(46px,1fr))] gap-y-16 gap-x-10 mt-16">
                    {partners.map((p, idx) => <a className='first-of-type:col-[1/-1] first-of-type:aspect-auto aspect-square rounded-16 first-of-type:rounded-0 overflow-hidden border first-of-type:border-none border-gray-200 ' key={idx} href={p.link}>
