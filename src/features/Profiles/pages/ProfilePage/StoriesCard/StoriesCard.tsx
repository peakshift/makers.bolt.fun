...
export default function StoriesCard({ stories, isOwner, onlyMd }: Props) {
  return (
    <div>
      <p className="text-body2 font-bolder text-center">
        ✍️ Stories ({stories.length})
      </p>
      {stories.length > 0 && (
        <ul className="">
          {stories.map((story) => (
            <li
              key={story.id}
              className="py-24 border-b-[1px] border-gray-200 last-of-type:border-b-0  "
            >
              <Link
              ...
              <div className="flex flex-wrap items-center gap-8 text-body5 mt-8">
                ...
                {story.tags.length > 3 && (
                  <Badge size="sm">+{story.tags.length - 3}</Badge>
                )}
              </div>
            </li>
          ))}
      ...
      {stories.length === 0 && (
        <div className="flex flex-col gap-16 mt-24">
          <p className="text-body4 text-gray-600">😐 No Stories Added Yet</p>
          {/* <p className="text-body5 text-gray-500">
                        The maker have not written any stories yet
                    </p> */}
          {isOwner && (
            <Button href={createRoute({ type: "write-story" })} color="primary">
