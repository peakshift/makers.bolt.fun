...
export default function WalletKey({
  ...
  const CONFIRM_DELETE_WALLET = useMemo(
    () =>
      createAction<{ confirmed?: boolean }>(
        `CONFIRM_DELETE_WALLET_${walletKey.key.slice(0, 10)}`
      )({}),
    [walletKey.key]
  );
  ...
  const onConfirmDelete = useCallback(
    ({ payload: { confirmed } }: typeof CONFIRM_DELETE_WALLET) => {
  ...
  return (
    <li key={walletKey.key} className="bg-gray-100 rounded">
      <div className="flex gap-16 items-center">
        <div className="flex items-center flex-grow relative min-w-0">
          ...
          <input
            ...
            type="text"
            value={name}
            className="input-text overflow-hidden text-ellipsis focus:border focus:border-solid focus:bg-white rounded-8 border-gray-800 py-8 my-4"
              ...
              <>
                ...
                {visible && (
                  <div
                    ...
                    {...getTooltipProps({
                      className:
                        "tooltip-container !bg-gray-900 !text-white text-body5 !rounded-12 !p-12",
                    })}
                  >
                    <div {...getArrowProps({ className: "tooltip-arrow" })} />
                    You're now logged-in with this wallet. <br /> To remove it,
                    login to your account with a different wallet.
                  </div>
                )}
              </>
            )}
