...
const CONFIRM_DELETE_STORY = createAction<{ confirmed?: boolean }>(
  "DELETE_STORY_CONFIRMED"
)({});
...
export const useUpdateStory = (story: Story) => {
    ...
    dispatch(
      stageStoryEdit({
        ...
        cover_image: story.cover_image
          ? { id: null, name: null, url: story.cover_image }
          : null,
      })
    );
  ...
  const onConfirmDelete = useCallback(
    ({ payload: { confirmed } }: typeof CONFIRM_DELETE_STORY) => {
      if (confirmed) deleteMutation();
    },
    [deleteMutation]
  );
