...
const schema = yup
  .object({
    ...
    deadline: yup.date().typeError("Deadline must be a valid date").required(),
    bounty_amount: yup
      .number()
...
interface IFormInputs {
  ...
  tags: NestedValue<Pick<Tag, "id" | "title">[]>;
  ...
  return (
    <FormProvider {...formMethods}>
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="bg-white shadow-lg rounded-8 overflow-hidden">
          <div className="p-32">
            {/* <Controller
                            control={control}
                            name="cover_image"
            ...
            <p className="text-body5 mt-16">Title</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-24 mt-16">
              <div>
                <p className="text-body5">Bounty Amount</p>
              ...
              <div>
                <p className="text-body5">Deadline</p>
                <Controller
                  name="deadline"
                  control={control}
                  render={({ field }) => (
                    <DatePicker {...field} className="mt-8" />
                  )}
                />
            ...
            <p className="text-body5 mt-16">Tags</p>
            <Controller
              control={control}
              name="tags"
              render={({ field: { onChange, value, onBlur } }) => (
            ...
            {errors.tags && (
              <p className="input-error">{errors.tags.message}</p>
            )}
          </div>
          <ContentEditor
          ...
          {errors.body && (
            <p className="input-error py-8 px-16">{errors.body.message}</p>
          )}
        </div>
