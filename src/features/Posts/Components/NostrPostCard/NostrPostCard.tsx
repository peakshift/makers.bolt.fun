...
export default function NostrPostCard({ post, author }: Props) {
  ...
  const imgIdx = post.content.search(EXTRACT_IMAGE_FROM_CONTENT_REGEX);
  const imgUrl = imgIdx !== -1 ? post.content.slice(imgIdx) : null;

  const contentWithoutImg =
    imgIdx !== -1 ? post.content.slice(0, imgIdx) : post.content;
      ...
      <Card className="overflow-hidden mt-8 flex flex-col gap-12">
        <Link to={"#"}>
          {imgUrl && (
            ...
            <img
              src={imgUrl}
              className="h-[200px] w-full object-cover rounded-8 mb-16"
              alt=""
            />
            // </Link>
          )}
