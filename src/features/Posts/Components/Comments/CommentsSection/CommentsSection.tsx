...
export default function CommentsSection({ type, id }: Props) {
  ...
  const { commentsTree, postComment, connectionStatus, retryConnection } =
    useComments({ type, id });
    ...
    <Card onlyMd>
      <div className="flex flex-wrap justify-between">
        ...
        {connectionStatus.status === "Connected" && (
          <div className="bg-green-50 text-green-500 text-body5 font-medium py-4 px-12 rounded-48">
            {" "}
            &#8226;{" "}
        ...
        {connectionStatus.status === "Connecting" && (
          <div className="bg-amber-50 text-amber-500 text-body5 font-medium py-4 px-12 rounded-48">
            {" "}
            &#8226;{" "}
            <span className="hidden md:inline">Connecting to relays</span> ⌛
          </div>
        )}
        {connectionStatus.status === "Not Connected" && (
          <div className="bg-red-50 text-red-500 text-body5 font-medium py-4 px-12 rounded-48">
            {" "}
            &#8226; <span className="hidden md:inline">
              Disconnected...
            </span>{" "}
        ...
        <div className="mt-24 relative">
          <div className={!user ? "blur-[2px]" : ""}>
            <AddComment
              ...
              avatar={
                user?.avatar ??
                "https://api.dicebear.com/7.x/bottts/svg?seed=default.svg"
              }
            />
          </div>
          {!user && (
