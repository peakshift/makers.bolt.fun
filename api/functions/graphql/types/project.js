  ...
  definition(t) {
    ...
    t.nonNull.field("category", {
      ...
      resolve: (parent) => {
        return (
          parent.category ||
          prisma.project.findUnique({ where: { id: parent.id } }).category()
        );
      },
    });
    ...
    t.nonNull.list.nonNull.field("awards", {
      ...
      resolve: (parent) => {
        return (
          parent.awards ||
          prisma.project.findUnique({ where: { id: parent.id } }).awards()
        );
      },
    });
    ...
    t.nonNull.list.nonNull.field("tags", {
      ...
      resolve: (parent) => {
        return (
          parent.tags ||
          prisma.project.findUnique({ where: { id: parent.id } }).tags()
        );
      },
    });
    ...
    t.nonNull.list.nonNull.field("capabilities", {
      ...
      resolve: async (parent) => {
        return (
          parent.capabilities ||
          prisma.project.findUnique({ where: { id: parent.id } }).capabilities()
        );
      },
    });
      ...
      resolve: async (parent, _, ctx) => {
        ...
        const role = (
          await prisma.projectMember.findUnique({
            where: {
              projectId_userId: { projectId: parent.id, userId: user.id },
            },
          })
        )?.role;
        ...
        if (role === ROLE_ADMIN)
          return [
            PROJECT_PERMISSIONS.UpdateMembers,
            PROJECT_PERMISSIONS.UpdateInfo,
          ];
  ...
  definition(t) {
    t.field("createProject", {
      ...
      async resolve(_root, args, ctx) {
        ...
        if (hashtagTaken)
          throw new ApolloError("Hashtag already used by another project");
        ...
        const coverImage = hostedImages.find(
          (img) => img.provider_image_id === cover_image.id
        );
        if (!coverImage)
          throw new ApolloError(
            "Cover image wasn't uploaded successfully, please try again"
          );
        ...
        const thumbnailImage = hostedImages.find(
          (img) => img.provider_image_id === thumbnail_image.id
        );
        if (!thumbnailImage)
          throw new ApolloError(
            "Thumbnail image wasn't uploaded successfully, please try again"
          );
        ...
        const screenshots_ids = hostedImages
          .filter((img) =>
            screenshots.some((s) => s.id === img.provider_image_id)
          )
          .map((img) => img.id);
  ...
  definition(t) {
    t.field("updateProject", {
      ...
      async resolve(_root, args, ctx) {
        ...
        if (
          project.members.find((m) => m.userId === user.id)?.role === ROLE_MAKER
        ) {
        ...
        if (
          project.members.find((m) => m.userId === user.id)?.role === ROLE_ADMIN
        ) {
          // Changing Makers
        ...
        for (const screenshot of screenshots) {
          if (screenshot.id) {
            const newScreenshot = hostedImages.find(
              (img) => img.provider_image_id === screenshot.id
            );
            ...
            const newScreenshot = oldScreenshots.find(
              (s) => s.url === screenshot.url
            );
            if (newScreenshot) {
              final_screenshots_ids.push(newScreenshot.id);
            }
        ...
        const screenshotsIdsToDelete = project.screenshots_ids.filter(
          (x) => !final_screenshots_ids.includes(x)
        );
  ...
  definition(t) {
    t.field("deleteProject", {
      ...
      async resolve(_root, args, ctx) {
        ...
        if (
          project.members.find((m) => m.userId === user.id)?.role !== ROLE_OWNER
        )
          throw new ApolloError(
            "You don't have the right to delete this project"
          );
